# image sources listed in ~TP/Resources/sources.txt
# This file contains the draw functions for the main singleplayer game.
...
def drawStatusBoard(self, screen):
    # status board is the top left board in game

    screen.blit(self.statusBoardImage, (0,0))
    diff = self.aiDiff
    if diff == 'medium': diff = 'med'
    line1 = 'Pot: %d | Raise: %d | %s' % (sum(self.status.betList),
            self.status.tempBet, diff)
...
def drawGameButtons(self, screen):
    # button drawing wrapper function
...
def drawPlayerCards(self, screen):
    ...
    for (i, player) in enumerate(playerList):
        for (j, card) in enumerate(player.hand[:2]): # only show 2 cards
            (x,y) = cardPos[i]
            if self.hideCards == True:
                if i == 0: # human player
                    suit, value = card
                else: suit = value = 'back'
            else:
                suit, value = card
...
def drawPlayerStatus(self, screen):
    # draws the red/blue squares behind the player's cards
    playerList = self.players
    ...
    for (i, player) in enumerate(playerList):
        ...
        if i == self.status.currentPlayer and not playerList[i].foldStatus: 
            r, g, b = (255, 0, 0)
